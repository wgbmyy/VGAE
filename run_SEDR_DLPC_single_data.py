...
parser.add_argument('--cell_feat_dim', type=int, default=160, help='Dim of PCA') #√
parser.add_argument('--feat_hidden1', type=int, default=100, help='Dim of DNN hidden 1-layer.')
parser.add_argument('--feat_hidden2', type=int, default=20, help='Dim of DNN hidden 2-layer.')
parser.add_argument('--gcn_hidden1', type=int, default=32, help='Dim of GCN hidden 1-layer.')
parser.add_argument('--gcn_hidden2', type=int, default=20, help='Dim of GCN hidden 2-layer.')
parser.add_argument('--p_drop', type=float, default=0.2, help='Dropout rate.')
parser.add_argument('--using_dec', type=bool, default=True, help='Using DEC loss.')
parser.add_argument('--using_mask', type=bool, default=False, help='Using mask for multi-dataset.')
parser.add_argument('--feat_w', type=float, default=10, help='Weight of DNN loss.')
parser.add_argument('--gcn_w', type=float, default=5, help='Weight of GCN loss.')
parser.add_argument('--dec_kl_w', type=float, default=10, help='Weight of DEC loss.')
parser.add_argument('--gcn_lr', type=float, default=0.0001, help='Initial GNN learning rate.')
parser.add_argument('--gcn_decay', type=float, default=0.01, help='Initial decay rate.')
parser.add_argument('--dec_cluster_n', type=int, default=5, help='DEC cluster number.')
parser.add_argument('--dec_interval', type=int, default=3, help='DEC interval nnumber.')
...
for proj_idx in range(len(proj_list)):
   ...
   #torch.cuda.manual_seed_all(1)
    ...
    A_pred, q, z = sedr_net.process()

    np.savez(f'{params.save_path}/SEDR_result.npz', sedr_feat=z, params=params)
    key_added = "SEDR"
    embeddings = pd.DataFrame(z)
    embeddings.index = adata_h5.obs_names
    adata_h5.obsm[key_added] = embeddings.loc[adata_h5.obs_names,].values

    # ######## Result plot
    adata_sedr = anndata.AnnData(z)
    ...
    sc.pl.spatial(adata_sedr, img_key="hires", color=['SEDR_leiden'], title=['VGAE(ARI=%.2f)' % ARI], show=False)
    plt.savefig(f'{params.save_path}/VGAE_leiden_plot.jpg', dpi=600)
    plt.show()
    sc.pl.spatial(adata_sedr, color="SEDR_leiden", title=['VGAE(ARI=%.2f)' % ARI], legend_fontsize=10,
                  na_in_legend=False)
    ...
    sc.pl.umap(used_adata, color=["pred", "Ground_Truth"],
               title=['VGAE', 'Ground_Truth'], legend_loc='right margin',show=False)
    plt.savefig(f'{params.save_path}/VGAE_umap_plot.jpg',dpi=600)
    plt.show()
    ...
    sc.pl.paga_compare(used_adata, legend_fontsize=10, frameon=True, size=20,
                       title='VGAE', legend_fontoutline=2, show=False)
    plt.savefig(f'{params.save_path}/VGAE_paga_plot.jpg',dpi=600)
    plt.show()

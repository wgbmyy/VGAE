...
class SEDR(nn.Module):
    def __init__(self, input_dim, params):
        ...
        self.alpha = 1.0
        # self.latent_dim = params.gcn_hidden2
        self.latent_dim = params.gcn_hidden1
        ...
        self.gc1 = GraphConvolution(input_dim, params.gcn_hidden1, params.p_drop, act=F.relu)
        ...
        # DEC cluster layer
        self.cluster_layer = Parameter(torch.Tensor(params.dec_cluster_n, params.gcn_hidden2))
    ...
    def encode(self, x, adj):
        ...
        return self.gc2(hidden, adj), self.gc3(hidden, adj)
    ...
    def reparameterize(self, mu,logvar):
        if self.training:
            ...
            # gaussian_noise = torch.randn(x.size(0), 32)
            # sampled_z = gaussian_noise * torch.exp(logvar) + mu
        else:
            return mu
    ...
    def forward(self, x, adj):
        ...
        # de_feat = self.decoder(gnn_z)
        A_pred = self.dc(gnn_z)


        # DEC clustering
        # q = 1.0 / (1.0 + torch.sum(torch.pow(z.unsqueeze(1) - self.cluster_layer, 2), 2) / self.alpha)
        q = 1.0 / (1.0 + torch.sum(torch.pow(gnn_z.unsqueeze(1) - self.cluster_layer, 2), 2) / self.alpha)
        q = q.pow((self.alpha + 1.0) / 2.0)
        q = (q.t() / torch.sum(q, 1)).t()

        # return z, mu, logvar, de_feat, q, feat_x, gnn_z
        return A_pred, mu, logvar,q,gnn_z

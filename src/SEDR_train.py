...
class SEDR_Train:
    ...
    def train_without_dec(self):
        ...
        for epoch in range(self.epochs):
            ...
            A_pred, mu, logvar,q,z= self.model(self.node_X, self.adj_norm)


            loss = gcn_loss(preds=A_pred, labels=self.adj_label, mu=mu,
    ...
    def process(self):
        self.model.eval()
        A_pred, _ , _ ,q,gnn_z = self.model(self.node_X, self.adj_norm)
        # latent_z = latent_z.data.cpu().numpy()
        A_pred = A_pred.data.cpu().numpy()
        q = q.data.cpu().numpy()
        # feat_x = feat_x.data.cpu().numpy()
        gnn_z = gnn_z.data.cpu().numpy()
        return A_pred, q, gnn_z

    def train_with_dec(self):
        # initialize cluster parameter
        self.train_without_dec()
        kmeans = KMeans(n_clusters=self.params.dec_cluster_n, n_init=self.params.dec_cluster_n, random_state=42)
        _,_,test_z = self.process()
        ...
        for epoch_id in range(self.epochs):
            ...
            self.optimizer.zero_grad()
            A_pred, mu, logvar, out_q, _,  = self.model(self.node_X, self.adj_norm)
            loss_gcn = gcn_loss(preds=A_pred, labels=self.adj_label, mu=mu,
                                logvar=logvar, n_nodes=self.params.cell_num, norm=self.norm_value, mask=self.adj_label)
            # loss_rec = reconstruction_loss(de_feat, self.node_X)
            # clustering KL loss
            loss_kl = F.kl_div(out_q.log(), torch.tensor(tmp_p).to(self.device)).to(self.device)
            loss = self.params.gcn_w * loss_gcn + self.params.dec_kl_w * loss_kl
            loss.backward()
            self.optimizer.step()
